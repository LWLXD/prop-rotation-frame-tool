...
const GLOBAL_OBJECT_LOCK_PROMPT = [
  ...
  "Preserve a casual Western cartoon mobile game prop icon style: clean simplified shapes, large smooth readable surfaces, chunky rounded bevels, saturated friendly colors, soft painted highlights, and a polished toy-like surface.",
  "The object should feel like a smooth Euro-style casual game prop: rounded, clean, slightly inflated, soft plastic or rubber-like, with broad continuous surfaces and simple readable shading.",
  "Keep the material simple and stylized, like a casual puzzle or match-3 game prop icon, not realistic metal, glass, stone, leather, cloth, paper, clay, organic material, or physically complex PBR material.",
  "When inferring unseen sides or the back, continue the same simplified cartoon icon material from the main image with smoother, cleaner surfaces than the visible side if needed.",
  "Unseen faces should be smooth and rounded, with no wrinkles, folds, creases, dents, grooves, seams, cracks, scratches, grime, rough texture, tiny ridges, noisy details, complex reflections, or heavy specular detail.",
...
const HORIZONTAL_360_PROMPT = [
  ...
  "Design the clip for perfect loop playback: the last frame should connect back to the first frame with no visible pop, jump cut, fade, dissolve, lighting shift, scale change, position shift, or pose mismatch.",
  "Do not add a separate intro, ending hold, fade-in, fade-out, slow stop, bounce-back, or reset motion; the rotation should continue smoothly if the video is played repeatedly.",
  "The final moment should not look like an ending. It should look like the next frame would naturally continue into the first frame of the same rotating loop.",
